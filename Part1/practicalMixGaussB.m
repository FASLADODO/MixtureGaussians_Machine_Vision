...
nIter = 20;    % more iterations the better the fits will be
for (cIter = 1:nIter)
   ...
   % LOOK AT THE MoGCribSheet.pdf, and implement all of the steps there!!!!
   
   for (cData = 1:nData)
        %fill in column of 'hidden' - caculate posterior probability that
        %this data point came from each of the Gaussians
        %replace this:
        for h = 1:k
            l(h) = mixGaussEst.weight(h)*calcGaussianProb(data(cData),mixGaussEst.mean(h),mixGaussEst.cov(h));
        end
        responsibilities(:,cData) = l/(sum(l));
   end;
   ...
   for (cGauss = 1:k) 
        ...
        sum_resp = sum(transpose(responsibilities));
        mixGaussEst.weight(cGauss) = sum_resp(cGauss)/sum(sum(responsibilities)); 
   
        %Update mean parameters mixGauss.mean by weighted average
        %where weights are given by posterior probability associated with
        %Gaussian.  Replace this:
        mixGaussEst.mean(cGauss) = responsibilities(cGauss,:)*transpose(data)/sum(responsibilities(cGauss,:));
        ...
        mixGaussEst.cov(1,1,cGauss) = responsibilities(cGauss,:).*(data-mixGaussEst.mean(cGauss))*transpose(data-mixGaussEst.mean(cGauss))/sum(responsibilities(cGauss,:));
   end;
...
for(cData = 1:nData)
    thisData = data(:,cData); 
    like = 0;
    %like = calcGaussianProb(thisData,mixGaussEst.mean(1),mixGaussEst.cov(1))*mixGaussEst.weight(1)+calcGaussianProb(thisData,mixGaussEst.mean(2),mixGaussEst.cov(2))*mixGaussEst.weight(2);
    for (h = 1:mixGaussEst.k)
...
for(cData = 1:nData)
    ...
    %calculate contribution to bound of this datapoint
    boundValue1 = thisQ(1)*log(mixGaussEst.weight(1)*calcGaussianProb(thisData,mixGaussEst.mean(1),mixGaussEst.cov(1))/thisQ(1));
    boundValue2 = thisQ(2)*log(mixGaussEst.weight(2)*calcGaussianProb(thisData,mixGaussEst.mean(2),mixGaussEst.cov(2))/thisQ(2));
    boundValue= boundValue1+boundValue2;
    %add to total log like
